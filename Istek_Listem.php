...
class Istek_Listem extends CI_Controller
{
	...
	{
		$veri = [
			'sayfa_adi' => 'uye/hesabim/istek-listem',
			'sayfa_title' => 'İstek Listem - ' . siteayar()->site_baslik,
	...
	public function listeyi_cek()
	{
		...
		$istek_listesi = $this->db->get_where('istek_listesi', array("uye_id" => uye_oturum_kontrol()->id));
		...
		foreach ($istek_listesi->result() as $istek) {
			...
			if ($urun) {
                ...
                if($urun_fiyat == 0){ $urun_fiyat = "STOK YOK";}
                else{ $urun_fiyat = number_format($urun->fiyat, 2, '.', ',').' ₺';}


				$html .= '
					<tr>
						<td class="text-center">
							<a href="'.$m_urun->kategori->seo . '/'  . $urun->seo . '"><img width="47" src="images/urunler/' . $urun_resimleri[0] . '" alt="#" /></a>
						</td>
						...
						<td class="text-center">
							<a class="button" href="'.$m_urun->kategori->seo . '/' . $urun->seo. ' ">
		...
		$result = [
			'html' => $html,
			'count' => $this->Uye_model->istek_listesi_sayisi(uye_oturum_kontrol(1)->id)
		];
	...
	public function sil()
	{
		...
		$kontrol = $this->db->get_where('istek_listesi', array("urun_id" => $urun_id, 'uye_id' => uye_oturum_kontrol()->id));

		if ($kontrol->num_rows() > 0) {
			$this->db->delete("istek_listesi", array("urun_id" => $urun_id, 'uye_id' => uye_oturum_kontrol()->id));
		}
	}
}
